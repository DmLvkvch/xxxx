...
private slots:
    void messageReceived(const Message & message);///<слот на полученное сообщения
    void messageSended(const Message & message);///<слот на отправленное сообщения
    void change();///<не нужен,
private:
    QString _name;
    QString _version;
    QIcon _icon;
